...
export const ChatHistory = ({ id }: ChatHistoryProps) => {
  ...
  const users = useMemo(() => {
    ...
    (normalized ?? []).forEach((h) => {
      const uname = h.username || h.user_id || "Неизвестный";
      if (h.user_id) unique.set(h.user_id, uname);
      else unique.set(uname, uname);
    });
  ...
  const historyIds = filteredHistories.map((h) => h.id || "unknown");
  const currentHistoryId = selectedHistory ?? historyIds[0];
  const currentHistory = filteredHistories.find(
  ...
  const needSelectUser = !selectedUser;
